(()=>{
 'use strict';
 if(window.__btvRecoveryV58)return;window.__btvRecoveryV58=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v58';
  ...
  if(!reset){
   ...
   const url=new URL(location.href);url.searchParams.set('v','55');url.searchParams.delete('refresh');url.searchParams.delete('home');url.searchParams.delete('screen');location.replace(url.href);return;
  }
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v58.js?v=58')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  const shell=document.getElementById('appShell');if(!shell||shell.hidden||shell.dataset.recoveredV58==='yes')return false;
  shell.dataset.recoveredV58='yes';
  ...
  const navigation=performance.getEntriesByType?.('navigation')?.[0];if(navigation?.type==='reload'&&typeof window.openScreen==='function')window.openScreen('home');
  restoreDashboard();return true;
 }
 function startRecovery(){
  installBrand();const shell=document.getElementById('appShell');if(!shell)return;
  if(restoreSignedInContent())return;
  const observer=new MutationObserver(()=>{if(restoreSignedInContent())observer.disconnect()});observer.observe(shell,{attributes:true,attributeFilter:['hidden']});
 }
 document.addEventListener('click',event=>{const home=event.target.closest('[data-open="home"],.brand'),journey=event.target.closest('[data-open="checklist"]'),costs=event.target.closest('[data-open="costs"]');if(home)setTimeout(restoreDashboard,0);if(journey&&typeof window.renderChecklist==='function')setTimeout(window.renderChecklist,0);if(costs&&typeof window.renderCosts==='function')setTimeout(window.renderCosts,0)},true);
 document.readyState==='loading'?document.addEventListener('DOMContentLoaded',startRecovery,{once:true}):startRecovery();
})();
