(()=>{
  ...
  function loading(dialog){dialog.innerHTML=`<main class="coinApp178 coinLoading178" aria-busy="true"><img class="coinMark178" src="favicon-192-v281.png" width="42" height="42" style="object-fit:contain;background:transparent" alt="Beyond The Visa"><h2>Opening your Beyond Coins wallet</h2><p>Checking your secure balance and latest activity…</p></main>`}
  function errorView(dialog,error){dialog.innerHTML=`<main class="coinApp178 coinError178"><button data-close aria-label="Close">×</button><img class="coinMark178" src="favicon-192-v281.png" width="42" height="42" style="object-fit:contain;background:transparent" alt="Beyond The Visa"><h2>We could not open your wallet</h2><p>${esc(error?.message||"Please try again.")}</p><button data-retry>Try again</button></main>`;dialog.querySelector("[data-close]").onclick=()=>dialog.close();dialog.querySelector("[data-retry]").onclick=()=>open(true)}
  ...
  function render(dialog,data,active="overview"){
    ...
    dialog.innerHTML=`<main class="coinApp178">
      <header class="coinTop178"><div class="coinBrand178"><img class="coinMark178" src="favicon-192-v281.png" width="42" height="42" style="object-fit:contain;background:transparent" alt="Beyond The Visa"><div><small>BEYOND THE VISA</small><b>Beyond Coins</b></div></div><div class="coinTopActions178"><span class="coinLive249"><i aria-hidden="true"></i>Ledger online</span><span class="coinLevelPill178">${esc(data.level?.badge||"Explorer")}</span><button data-refresh aria-label="Refresh wallet">↻</button><button data-close aria-label="Close wallet">×</button></div></header>
