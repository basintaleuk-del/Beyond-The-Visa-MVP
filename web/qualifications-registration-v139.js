(() => {
  ...
  async function loadDocuments(userId) {
    ...
    state.documents = results.flatMap((result, index) => (result.data || []).filter((file) => file.name !== ".emptyFolderPlaceholder").map((file) => ({ path: `${userId}/${categories[index]}/${file.name}`, name: file.name.replace(/^\d+-/, "") })));
  }
  ...
  function badge(done, started = false) {
    const text = done ? "Complete" : started ? "Incomplete" : "Not started";
    return `<span class="qrBadge139 ${done ? "complete" : ""}">${text}</span>`;
  }
  ...
  function section(title, description, statusMarkup, body, open = false) {
    return `<details class="qrSection139" ${open ? "open" : ""}><summary><div><h2>${title}</h2><p>${description}</p></div>${statusMarkup}<i aria-hidden="true">⌄</i></summary><div class="qrSectionBody139">${body}</div></details>`;
  }

  function primarySection(done) {
    const p = state.profile;
    ...
    return section("Primary Qualification", "Your main professional training record.", badge(done, Object.keys(p).length > 0), form, !done);
  }

  function registrationsSection(done) {
    ...
    return section("Registration History", "Add every current or previous professional registration.", badge(done), form, Boolean(edit.id));
  }

  function practiceSection(done) {
    ...
    return section("Practice History", "Record employers, clinical areas and estimated practice hours.", badge(done), form, Boolean(edit.id));
  }

  function assessmentsSection(done) {
    ...
    return section("Examinations & Assessments", "Track professional and language examinations without assuming they apply to every pathway.", badge(done), form, Boolean(edit.id));
  }

  function englishSection(done) {
    ...
    return section("English-Language Evidence", "Record test results and associate a document from My Documents.", badge(done, Boolean(p.english_test_type)), form);
  }

  function documentsSection(done) {
    ...
    return section("Supporting Documents", "Uses your existing private My Documents vault.", badge(done), `<ul class="qrDocuments139">${files}</ul><button type="button" class="qrDocumentsButton139" data-open-documents>Open My Documents</button>`);
  }
