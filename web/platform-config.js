...
if('serviceWorker'in navigator){let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading||sessionStorage.getItem('btv-sw-v253-reloaded'))return;reloading=true;sessionStorage.setItem('btv-sw-v253-reloaded','1');location.reload()});navigator.serviceWorker.register('./sw.js?v=253',{updateViaCache:'none'}).then(reg=>{reg.update();if(reg.waiting)reg.waiting.postMessage({type:'BTV_SKIP_WAITING'})}).catch(console.warn)}
