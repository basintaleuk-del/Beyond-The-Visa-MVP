      ...
      main.innerHTML = `
        ...
        <div class="newsDesk228">
          <section class="newsEditorial228"><header><div><small>LATEST REPORTING</small><h2>News desk</h2></div><span>${esc(names[selected])}</span></header><div class="newsGrid228">
            ${items.map((item, index) => `<a href="${esc(item.canonical_url)}" target="_blank" rel="noopener noreferrer" class="${index === 0 ? "featured" : ""}"><small>${esc(item.publisher || names[item.country_code] || "Immigration news")} &middot; ${date(item.published_at)}</small><h3>${esc(item.title)}</h3><p>${esc(item.summary || "Open the original report for full details.")}</p><span>Read original report &nearr;</span></a>`).join("") || '<div class="newsEmpty228"><b>No current headlines found.</b><p>Please check again after the next daily update.</p></div>'}
          </div></section>
