(()=>{
  ...
  function install(){cleanNavigation();if(!shell())return;cleanNavigation();const nav=$('.sidebar nav');if(!nav.dataset.usersDelegateV303){nav.dataset.usersDelegateV303='1';nav.addEventListener('click',event=>{const button=event.target.closest('[data-tab="cmsUsers"]');if(!button||button.hidden)return;event.preventDefault();event.stopImmediatePropagation();openUserManagement()},true)}addEventListener('online',()=>{if($('#cmsUsers')?.classList.contains('active'))load()});new MutationObserver(cleanNavigation).observe(nav,{childList:true,subtree:true});}
