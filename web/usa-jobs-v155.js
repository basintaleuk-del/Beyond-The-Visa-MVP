(() => {
  ...
  function card(row, compact = false) {
    return `<article class="usaJob155"><span>${esc(row.nursing_specialty || "Nursing")} · USAJOBS</span><h3><a href="/jobs/usa/${row.id}" data-usa-detail="${row.id}">${esc(row.job_title)}</a></h3><strong>${esc(row.agency || row.employer_name)}</strong>${row.department ? `<small>${esc(row.department)}</small>` : ""}<div class="usaBadges155"><b>${esc(row.remote_status === "not_stated" ? "Workplace arrangement not stated" : row.remote_status)}</b><b>${esc(row.schedule || row.employment_type || "Schedule not stated")}</b></div>${compact ? "" : `<p>${esc(row.description || "Open the vacancy for the complete federal job description and qualifications.")}</p>`}<dl class="usaFacts155"><div><dt>Location</dt><dd>${esc(row.location_display || [row.city, row.state].filter(Boolean).join(", ") || "United States")}</dd></div><div><dt>Salary</dt><dd>${esc(money(row))}</dd></div><div><dt>Opening date</dt><dd>${esc(date(row.opening_date || row.date_posted))}</dd></div><div><dt>Closing date</dt><dd>${esc(date(row.closing_date))}</dd></div><div class="wide"><dt>Who may apply</dt><dd>${esc(row.who_may_apply || "Review the official announcement")}</dd></div></dl><div class="usaActions155"><button data-usa-detail="${row.id}">View details</button><a href="${esc(row.canonical_application_url)}" target="_blank" rel="noopener noreferrer">View and apply on USAJOBS ↗</a><button data-save-usa="${row.id}">${state.saved.has(row.id) ? "Saved ✓" : "Save job"}</button></div></article>`;
  }
  ...
  async function openDetail(id, push = true) {
    ...
    try {
      ...
      element.innerHTML = `<article><button class="usaJobDetailClose155" data-close-usa aria-label="Close">×</button><header><span>${esc(job.nursing_specialty || "NURSING")} · ${esc(job.attribution_text)}</span><h1>${esc(job.job_title)}</h1><p>${esc(job.agency || job.employer_name)}${job.department ? ` · ${esc(job.department)}` : ""} · ${esc(job.location_display || [job.city, job.state].filter(Boolean).join(", ") || "United States")}</p></header><div class="usaDetailActions155"><button data-close-usa>← Back to USA jobs</button><a href="${esc(job.canonical_application_url)}" target="_blank" rel="noopener noreferrer">View and apply on USAJOBS ↗</a></div><dl class="usaDetailFacts155"><div><dt>Salary</dt><dd>${esc(money(job))}</dd></div><div><dt>Schedule</dt><dd>${esc(job.schedule || job.employment_type || "Not stated")}</dd></div><div><dt>Grade</dt><dd>${esc(job.grade || "Not stated")}</dd></div><div><dt>Opening date</dt><dd>${esc(date(job.opening_date || job.date_posted))}</dd></div><div><dt>Closing date</dt><dd>${esc(date(job.closing_date))}</dd></div><div><dt>Remote status</dt><dd>${esc(job.remote_status || "Not stated")}</dd></div></dl>${section("Who may apply", job.who_may_apply)}${section("Job description", job.description)}${section("Qualifications", job.qualifications)}${section("Requirements", job.requirements)}${section("Licence requirements", job.licence_requirements)}${section("Sponsorship evidence", job.sponsorship_evidence || (job.visa_sponsorship_status === "unclear" ? "Sponsorship status not confirmed. Check the official vacancy before applying." : ""))}<footer><p>${esc(job.attribution_text)}. Beyond the Visa is not the employer, recruiter or federal agency.</p><a href="${esc(job.canonical_application_url)}" target="_blank" rel="noopener noreferrer">View and apply on USAJOBS ↗</a><a href="${esc(job.source_job_url)}" target="_blank" rel="noopener noreferrer">View official vacancy</a></footer></article>`;
