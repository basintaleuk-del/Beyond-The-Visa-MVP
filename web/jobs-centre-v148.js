(() => {
  ...
  async function fetchAll(){
    ...
    for(let from=0;from<3000;from+=500){
      const result=await db().from("btv_jobs").select("*").in("status",ACTIVE_JOB_STATUSES).is("expired_at",null).eq("source_name","NHS Jobs").eq("opportunity_type","job").order("published_at",{ascending:false}).range(from,from+499);
  ...
  async function openDetail(row,{pushRoute=true,returnUrl}={}){
    const dialog=detailDialog(),source=row.canonical_url||row.source_url||row.application_url,applyFallback=row.application_url||source;
    ...
    let details={};
    try{let advertId="";try{advertId=new URL(source,location.origin).pathname.match(/\/jobadvert\/([^/]+)/i)?.[1]||"";}catch{}advertId=advertId||row.external_reference||row.external_id||"";const response=await fetch(`/api/job-details?id=${encodeURIComponent(advertId)}`);const payload=await response.json();if(!response.ok)throw new Error(payload.error||"Details could not be loaded.");details=payload;}catch(error){details={overview:row.summary,mainDuties:"The full duties and person specification could not be retrieved at this moment.",error:error.message,applyUrl:applyFallback,sourceUrl:source};}
    ...
    dialog.innerHTML=`<article><button class="nhsJobDetailClose150" data-close-job-detail aria-label="Close job details">×</button><header><span>${esc(LABELS[row.profession]||LABELS.other)} · NHS JOBS</span><h1>${esc(details.title||row.title)}</h1><p>${esc(details.employer||row.employer)}</p><div>${row.band?`<b>${esc(row.band)}</b>`:""}<b>${esc(row.employment_type||details.contract||"Contract not stated")}</b></div></header><div class="nhsJobDetailActions150"><button data-close-job-detail>← Back to listings</button>${apply?`<a href="${esc(apply)}" target="_blank" rel="noopener noreferrer">Apply</a>`:""}</div><dl class="nhsJobDetailFacts150"><div><dt>Salary</dt><dd>${esc(details.salary||row.salary_text||"Not stated")}</dd></div><div><dt>Location</dt><dd>${esc(locationText)}</dd></div><div><dt>Closing date</dt><dd>${esc(details.closingDate||date(row.closing_at))}</dd></div><div><dt>Reference</dt><dd>${esc(details.reference||row.external_reference||row.external_id||"Not stated")}</dd></div><div><dt>Published</dt><dd>${esc(details.datePosted||date(row.published_at))}</dd></div><div><dt>Sponsorship</dt><dd>${esc(sponsorship)}</dd></div></dl>${details.error?`<p class="nhsJobDetailNotice150">${esc(details.error)} You can still continue to the official advert.</p>`:""}${detailSection("Job summary",details.overview||row.summary)}${detailSection("Main duties of the job",details.mainDuties)}${detailSection("Job description and responsibilities",details.jobDescription)}${detailSection("About the employer",details.aboutEmployer)}${detailSection("Person specification",details.personSpecification)}${detailSection("Additional information",details.additionalInformation)}<section><h2>Employer contact</h2><p>${detailText([details.contactRole,details.contactName,details.contactEmail,details.contactPhone].filter(Boolean).join("\n")||"See the official advert for employer contact details.")}</p></section><footer><p>Your application continues on the original NHS Jobs application page, where the recruiting employer receives it.</p>${apply?`<a href="${esc(apply)}" target="_blank" rel="noopener noreferrer">Apply</a>`:""}${details.sourceUrl?`<a href="${esc(details.sourceUrl)}" target="_blank" rel="noopener noreferrer">View original advert</a>`:""}</footer></article>`;
    dialog.querySelectorAll("[data-close-job-detail]").forEach((button)=>button.onclick=()=>closeDetail(true));
  }

  function card(row){
    const apply=row.application_url||row.canonical_url||row.source_url,family=LABELS[row.profession]||LABELS.other,sponsorship=row.sponsorship_status==="confirmed"?"Sponsorship confirmed":row.sponsorship_status==="may_be_available"?"Sponsorship may be available":"",detailUrl=`/jobs/${encodeURIComponent(row.id)}`;
    return `<article class="nhsJob148" data-select-nhs-job="${row.id}" tabindex="0" role="link" aria-label="View ${esc(row.title)}"><div class="nhsJobTop148"><div><span>${esc(family)} · NHS Jobs</span><h3><a href="${detailUrl}" data-view-nhs-job="${row.id}">${esc(row.title)}</a></h3><p class="nhsJobEmployer148">${esc(row.employer)}</p></div><div class="nhsJobBadges148">${row.band?`<b>${esc(row.band)}</b>`:""}${sponsorship?`<b class="sponsor148">${esc(sponsorship)}</b>`:""}</div></div><p class="nhsJobSummary148">${esc(row.summary||"Open the vacancy details for the job description and person specification.")}</p><dl class="nhsJobFacts148"><div><dt>Location</dt><dd>${esc(row.location||row.city||"Not stated")}</dd></div><div><dt>Salary</dt><dd>${esc(row.salary_text||"Not stated")}</dd></div><div><dt>Contract</dt><dd>${esc(row.employment_type||"Not stated")}</dd></div><div><dt>Closing date</dt><dd>${date(row.closing_at)}</dd></div></dl><div class="nhsJobActions148"><a href="${detailUrl}" data-view-nhs-job="${row.id}">View details</a>${apply?`<a class="secondary150" href="${esc(apply)}" target="_blank" rel="noopener noreferrer">Apply</a>`:""}<button data-save-nhs-job="${row.id}">${view.saved.has(row.id)?"Saved ✓":"Save job"}</button><small>Ref: ${esc(row.external_reference||row.external_id||"Not stated")}</small></div></article>`;
  }
