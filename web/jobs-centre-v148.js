(() => {
  ...
  async function fetchAll(){
    ...
    for(let from=0;from<3000;from+=500){
      const result=await db().from("btv_jobs").select("*").in("status",ACTIVE_JOB_STATUSES).is("expired_at",null).in("source_name",["NHS Jobs","REED"]).eq("country_code","GB").eq("opportunity_type","job").order("published_at",{ascending:false}).range(from,from+499);
  ...
  const sourceApplyLink=(url,isReed)=>isReed?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">View and apply</a>`:`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">Apply</a>`;
  ...
  async function openDetail(row,{pushRoute=true,returnUrl}={}){
    const dialog=detailDialog(),source=row.canonical_url||row.source_url||row.application_url,applyFallback=row.application_url||source,isReed=row.source_name==="REED";
  ...
  function card(row){
    const apply=row.application_url||row.canonical_url||row.source_url,family=LABELS[row.profession]||LABELS.other,sponsorship=row.sponsorship_status==="confirmed"?"Sponsorship confirmed":row.sponsorship_status==="may_be_available"?"Sponsorship may be available":"",detailUrl=`/jobs/${encodeURIComponent(row.id)}`,isReed=row.source_name==="REED";
