(() => {
  ...
  const ACTIVE_JOB_STATUSES = ["published", "active", "closing_soon"];
  const PAGE_SIZE = 48;
  const view = { rows:[], saved:new Set(), visible:PAGE_SIZE, loaded:false, loading:false, checkedAt:null, filters:{...EMPTY_FILTERS} };
  ...
  async function fetchAll(){
    ...
    for(let from=0;;from+=500){
      const result=await db().from("btv_jobs").select("*").in("status",ACTIVE_JOB_STATUSES).is("expired_at",null).in("source_name",["NHS Jobs","REED","ADZUNA","jooble","careerjet"]).eq("country_code","GB").eq("opportunity_type","job").order("published_at",{ascending:false}).range(from,from+499);
  ...
  async function load(force=false){
    ...
    try{
      ...
      view.rows=rows; view.saved=new Set((saved.data||[]).map((item)=>item.job_id)); view.checkedAt=rows.map((row)=>row.last_checked_at).filter(Boolean).sort().at(-1)||null; view.visible=PAGE_SIZE; view.loaded=true; render();
  ...
  const sourceKey=(row)=>String(row.source_name||"NHS Jobs").toLowerCase();
  const dailySeed=()=>Number(new Date().toISOString().slice(0,10).replaceAll("-",""));
  const hash=(value,seed)=>{let result=(2166136261^seed)>>>0;for(const char of String(value)){result^=char.charCodeAt(0);result=Math.imul(result,16777619)>>>0;}return result;};
  function mixedSources(rows){
    const groups=new Map();
    rows.forEach((row)=>{const key=sourceKey(row);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(row);});
    const seed=dailySeed(),keys=[...groups.keys()].sort((a,b)=>hash(a,seed)-hash(b,seed));
    groups.forEach((items,key)=>items.sort((a,b)=>hash(`${a.id}|${a.external_id||""}`,seed+hash(key,seed))-hash(`${b.id}|${b.external_id||""}`,seed+hash(key,seed))));
    const mixed=[];let remaining=true,round=0;
    while(remaining){remaining=false;for(let offset=0;offset<keys.length;offset+=1){const key=keys[(offset+round)%keys.length],item=groups.get(key)?.[round];if(item){mixed.push(item);remaining=true;}}round+=1;}
    return mixed;
  }
  function filtered(){const f=view.filters,rows=view.rows.filter((row)=>{const text=`${row.title} ${row.employer} ${row.summary||""} ${row.location||""} ${row.external_reference||""}`.toLowerCase();return(!f.search||text.includes(f.search))&&(!f.profession||row.profession===f.profession)&&(!f.location||`${row.location||""} ${row.city||""} ${row.region||""}`.toLowerCase().includes(f.location))&&(!f.employer||String(row.employer||"").toLowerCase().includes(f.employer))&&(!f.contract||String(row.employment_type||"").toLowerCase().includes(f.contract))&&(!f.band||String(row.band||"").toLowerCase()===f.band)&&(!f.sponsorship||row.sponsorship_status==="confirmed"||row.sponsorship_status==="may_be_available");});return mixedSources(rows);}
  ...
  const sourceLabel=(row)=>row.source_name==="ADZUNA"?"Jobs by Adzuna":row.source_name==="REED"?"Reed":String(row.source_name).toLowerCase()==="jooble"?"Jooble":String(row.source_name).toLowerCase()==="careerjet"?"Careerjet":"NHS Jobs";
  ...
  function openStatDialog(kind){
    ...
    dialog.querySelectorAll("[data-stat-filter]").forEach((button)=>button.onclick=()=>{view.filters={...EMPTY_FILTERS,[button.dataset.statFilter]:button.dataset.statValue};view.visible=PAGE_SIZE;dialog.close();render();root().querySelector(".nhsJobsMain148")?.scrollIntoView({behavior:"smooth",block:"start"});});
    dialog.showModal();
  }
  async function openDetail(row,{pushRoute=true,returnUrl}={}){
    const dialog=detailDialog(),source=row.canonical_url||row.source_url||row.application_url,applyFallback=row.application_url||source,isAdzuna=row.source_name==="ADZUNA",isJooble=String(row.source_name).toLowerCase()==="jooble",isCareerjet=String(row.source_name).toLowerCase()==="careerjet",isReed=row.source_name==="REED"||isAdzuna||isJooble||isCareerjet,isExternal=isReed;
    ...
    if(isJooble)dialog.innerHTML=dialog.innerHTML.replace(/REED/g,"Jooble").replace(/Reed/g,"Jooble");
