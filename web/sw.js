const CACHE_NAME='beyond-the-visa-assets-v254';
...
function offlinePage(){return new Response(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#133e43"><title>Beyond The Visa — Reconnecting</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#f5f3ed;color:#183034;font-family:system-ui,sans-serif}.card{width:min(86%,420px);padding:30px;border-radius:24px;background:#fff;text-align:center;box-shadow:0 18px 50px #163e4318}.mark{width:72px;height:72px;margin:auto;display:block;object-fit:contain}h1{font:28px Georgia,serif}p{line-height:1.55;color:#6e7d7e}button{border:0;border-radius:13px;padding:12px 18px;background:#133e43;color:#fff;font-weight:800}</style></head><body><main class="card"><img class="mark" src="favicon-192-v281.png" width="72" height="72" alt="Beyond The Visa"><h1>Reconnecting…</h1><p>The secure sign-in page did not load on the first attempt. We will retry automatically when the connection is available.</p><button onclick="location.reload()">Try again now</button></main><script>let attempts=0;const retry=()=>{if(navigator.onLine&&attempts++<4)location.reload()};addEventListener('online',retry);setTimeout(retry,3000)</script></body></html>`,{status:503,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}})}
