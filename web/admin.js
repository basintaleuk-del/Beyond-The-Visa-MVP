...
async function loadAll(){
  const [profiles,cbt,nclex,ca,ce,na,ne,cbtActive,nclexActive,cbtPremium,nclexPremium]=await Promise.all([
    sb.from('profiles').select('*').order('created_at',{ascending:false}),
    readRecent('cbt_questions'),
    readRecent('nclex_questions'),
    ...
    sb.from('cbt_questions').select('id',{count:'exact',head:true}).eq('is_active',true),
    sb.from('nclex_questions').select('id',{count:'exact',head:true}).eq('is_active',true),
    sb.from('cbt_questions').select('id',{count:'exact',head:true}).eq('access_level','premium'),
    sb.from('nclex_questions').select('id',{count:'exact',head:true}).eq('access_level','premium')
  ]);
  ...
  state.metrics={cbtTotal:cbt.count||0,nclexTotal:nclex.count||0,cbtActive:cbtActive.count||0,nclexActive:nclexActive.count||0,premium:(cbtPremium.count||0)+(nclexPremium.count||0)};
  renderOverview({ca,ce,na,ne});renderCbt();renderNclex();renderUsers();renderAnalytics({ca,ce,na,ne});
}
function renderOverview(x){
  $('#statUsers').textContent=state.profiles.length;$('#statPremium').textContent=state.profiles.filter(p=>p.account_type==='premium').length;$('#statCbt').textContent=state.metrics.cbtTotal;$('#statNclex').textContent=state.metrics.nclexTotal;
...
async function saveEditor(e){e.preventDefault();if(e.submitter?.value==='cancel')return $('#editor').close();const {kind,item}=state.editing;const fd=new FormData(e.currentTarget),obj=Object.fromEntries(fd.entries());obj.is_active=fd.has('is_active');if(obj.is_active&&item?.quality_status!=='approved'&&!confirm('Publishing confirms that a qualified reviewer has checked this question, answers, rationale and current exam blueprint. Continue?'))return;if(obj.is_active){obj.quality_status='approved';obj.review_status='approved';obj.reviewed_at=new Date().toISOString();obj.standard_version=kind==='cbt'?'NMC Test of Competence blueprint (current 2026)':'NCSBN 2026 NCLEX-RN Test Plan';obj.blueprint_domain=kind==='cbt'?({'Infection Prevention':'Professional values','Professional Values':'Professional values','Documentation':'Communication and interpersonal skills','Prioritisation':'Leadership, management and team working'}[obj.subject]||'Nursing practice and decision making'):obj.client_need}if(kind==='nclex'){obj.correct_options=obj.correct_options.split(',').map(x=>x.trim().toUpperCase()).filter(Boolean);['option_e','option_f','test_strategy'].forEach(k=>{if(!obj[k])obj[k]=null})}const table=kind==='cbt'?'cbt_questions':'nclex_questions';let result=item?await sb.from(table).update(obj).eq('id',item.id).select().single():await sb.from(table).insert(obj).select().single();if(result.error){$('#editorMessage').textContent=result.error.message;return}const arr=kind==='cbt'?state.cbt:state.nclex;if(item){const i=arr.findIndex(x=>x.id===item.id);arr[i]=result.data}else arr.unshift(result.data);$('#editor').close();kind==='cbt'?renderCbt():renderNclex();renderOverview({ca:{count:0},ce:{data:[]},na:{count:0},ne:{data:[]}});}
