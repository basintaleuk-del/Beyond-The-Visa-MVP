...
async function ask(question){
  if(sending||!question)return;sending=true;const root=document.getElementById('assistant'),form=root?.querySelector('[data-zibur-form]'),input=root?.querySelector('[data-zibur-question]'),button=form?.querySelector('[type="submit"]');
...
function install(){
  ...
  const form=root.querySelector('[data-zibur-form]'),input=root.querySelector('[data-zibur-question]');input.addEventListener('input',()=>resize(input));input.addEventListener('keydown',event=>{if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();form.requestSubmit()}});form.onsubmit=event=>{event.preventDefault();const question=clean(input.value);if(question)ask(question)};
