(()=>{
 'use strict';
 if(window.__btvRecoveryV61)return;window.__btvRecoveryV61=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v61';
  ...
  if(!reset){
   ...
   const url=new URL(location.href);url.searchParams.set('v','61');url.searchParams.delete('refresh');url.searchParams.delete('home');url.searchParams.delete('screen');location.replace(url.href);return;
  }
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v61.js?v=61')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  const shell=document.getElementById('appShell');if(!shell||shell.hidden||shell.dataset.recoveredV61==='yes')return false;
  shell.dataset.recoveredV61='yes';
