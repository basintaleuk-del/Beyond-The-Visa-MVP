(()=>{
 'use strict';
 if(window.__btvRecoveryV57)return;window.__btvRecoveryV57=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v57';
  ...
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v57.js?v=57')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  const shell=document.getElementById('appShell');if(!shell||shell.hidden||shell.dataset.recoveredV57==='yes')return false;
  shell.dataset.recoveredV57='yes';
