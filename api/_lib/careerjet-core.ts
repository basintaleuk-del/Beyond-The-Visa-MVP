const { normalizeJob, plainText } = require("./global-jobs-core.cjs") as {
  normalizeJob: (input: Record<string, unknown>, source: Record<string, unknown>, now?: Date) => { job: CareerjetJob; errors: string[] };
  plainText: (value: unknown, max?: number) => string;
};
...
type CareerjetJob = Record<string, unknown> & {
  external_id: string;
  source_name: string;
...
const cache = new Map<string, { expiresAt: number; value: CareerjetResponse }>();
const delay = (ms: number) => new Promise<void>((resolve) => setTimeout(resolve, ms));

function credentialError(value: unknown): string | null {
  return typeof value === "string" && value.trim() ? null : "CAREERJET_AFFILIATE_ID is required.";
}

function countryFor(value: unknown): Country {
...
function safeLog(logger: Logger, event: Record<string, unknown>): void {
  logger({ provider: "careerjet", ...event });
}

function validateResponse(value: unknown): CareerjetResponse {
  ...
  const body = value as Partial<CareerjetResponse> & { type?: unknown; jobs?: unknown; hits?: unknown; pages?: unknown };
  ...
  return { ...body, type: "JOBS", jobs: body.jobs as CareerjetRawJob[], hits: Number(body.hits), pages: Number(body.pages) };
}
...
function retryAfterMs(response: Response, attempt: number): number {
  ...
  return Math.min(30_000, 600 * (2 ** attempt));
}
...
async function requestPage(options: {
  ...
  cacheTtlMs?: number;
}): Promise<{ body: CareerjetResponse; requestsMade: number; fromCache: boolean; httpStatus: number }> {
  ...
  if ((options.cacheTtlMs ?? 300_000) > 0 && cached && cached.expiresAt > Date.now()) return { body: cached.value, requestsMade: 0, fromCache: true, httpStatus: 200 };
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    const timeout = setTimeout(() => controller.abort(), Math.max(1000, Number(options.timeoutMs || 12_000)));
    try {
      requestsMade += 1;
      ...
      catch { throw Object.assign(new Error("Careerjet returned invalid JSON."), { status: 502, code: "CAREERJET_INVALID_JSON" }); }
      const body = validateResponse(parsed);
      if ((options.cacheTtlMs ?? 300_000) > 0) cache.set(cacheKey, { value: body, expiresAt: Date.now() + Number(options.cacheTtlMs ?? 300_000) });
      ...
      if ((isTimeout || !typed.status || typed.status >= 500) && attempt < retries) {
        const waitMs = Math.min(30_000, 600 * (2 ** attempt));
...
function salaryPeriod(value: unknown): string | null {
  return ({ Y: "year", M: "month", W: "week", D: "day", H: "hour" } as Record<string, string>)[String(value || "").toUpperCase()] || null;
}

function professionFor(value: unknown): string {
...
function stableExternalId(url: string): string {
  ...
  } catch { return plainText(url, 300); }
}

function mapJob(raw: CareerjetRawJob, countryKey: string, now = new Date()): { job: CareerjetJob; errors: string[] } {
  const country = countryFor(countryKey);
  const url = typeof raw.url === "string" ? raw.url : "";
  const location = plainText(raw.locations, 300);
  const text = `${plainText(raw.title, 400)} ${plainText(raw.description, 3000)} ${location}`;
...
function canonicalUrlKey(value: unknown): string {
  try { const url = new URL(String(value || "")); return `${url.hostname.toLowerCase()}${url.pathname.replace(/\/+$/, "").toLowerCase()}`; }
  catch { return ""; }
}

function duplicateKey(row: Record<string, unknown>): string {
  const clean = (value: unknown) => plainText(value, 400).toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
...
async function fetchCountryJobs(options: {
  affiliateId: string;
  country: string;
  keywordBatches?: readonly string[];
  maxPages?: number;
  pageSize?: number;
  maxJobs?: number;
  userIp?: string;
  ...
  wait?: (ms: number) => Promise<void>;
  cacheTtlMs?: number;
}): Promise<{ records: CareerjetJob[]; requestsMade: number; jobsReceived: number; duplicates: number; invalid: number; errors: string[] }> {
  countryFor(options.country);
  const now = options.now || new Date();
  const records: CareerjetJob[] = [], errors: string[] = [], seenUrls = new Set<string>(), seenIdentities = new Set<string>();
  ...
  outer: for (const keywords of batches) {
    for (let page = 1; page <= maxPages; page += 1) {
      ...
      for (const raw of result.body.jobs) {
        ...
        if (mapped.errors.length) { invalid += 1; errors.push(...mapped.errors.map((error) => `${plainText(raw.title, 80) || "Untitled"}: ${error}`)); continue; }
        const url = canonicalUrlKey(mapped.job.canonical_url), identity = duplicateKey(mapped.job);
        if ((url && seenUrls.has(url)) || (identity && seenIdentities.has(identity))) { duplicates += 1; continue; }
...
async function testConnection(options: Omit<Parameters<typeof requestPage>[0], "keywords" | "page" | "pageSize"> & { country?: string }): Promise<Record<string, unknown>> {
  try {
