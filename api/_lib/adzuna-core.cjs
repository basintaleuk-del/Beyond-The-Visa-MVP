...
function safeAdzunaUrl(value) {
  try {
    ...
    if (!/^https?:$/.test(url.protocol) || url.username || url.password || !(url.hostname === "adzuna.com" || url.hostname.endsWith(".adzuna.com"))) return null;
...
function normalizeItem(item, now = new Date()) {
  ...
  const locationDisplay = clean(item?.location?.display_name || area.slice(1).reverse().join(", "), 500) || "United States";
  ...
  const result = {
    ...
    city, state, location_display: locationDisplay, country: "United States", country_code: "US", destination_country: "United States of America",
    salary_min: number(item?.salary_min), salary_max: number(item?.salary_max), salary_currency: "USD", salary_period: null,
...
function requestUrl({ appId, appKey, keyword, page, resultsPerPage }) {
...
async function requestPage({ appId, appKey, keyword, page = 1, resultsPerPage = LIMITS.resultsPerPage, fetchImpl = fetch }) {
  let lastError;
  for (let attempt = 0; attempt < LIMITS.retries; attempt += 1) {
    try {
      const response = await fetchImpl(requestUrl({ appId, appKey, keyword, page, resultsPerPage }), { headers: { Accept: "application/json" }, signal: AbortSignal.timeout(LIMITS.timeoutMs) });
...
async function testConnection({ appId, appKey, fetchImpl = fetch }) {
  ...
  try {
    const response = await fetchImpl(requestUrl({ appId, appKey, keyword: "Registered Nurse", page: 1, resultsPerPage: 5 }), { headers: { Accept: "application/json" }, signal: AbortSignal.timeout(LIMITS.timeoutMs) });
...
async function fetchAdzunaJobs({ appId, appKey, pageBudget = LIMITS.pageBudget, sample = false, fetchImpl = fetch, now = new Date() }) {
  ...
  for (const term of searchPlan) {
    ...
    for (let page = 1; page <= term.pages && pagesFetched < Math.min(pageBudget, LIMITS.pageBudget); page += 1) {
      const payload = await requestPage({ appId, appKey, keyword: term.keyword, page, resultsPerPage: sample ? 5 : LIMITS.resultsPerPage, fetchImpl }); pagesFetched += 1;
      const results = Array.isArray(payload?.results) ? payload.results : []; raw.push(...results); if (!results.length) break;
    }
  }
  const byId = new Map(); for (const item of raw) { const row = normalizeItem(item, now); if (row) byId.set(row.external_id, row); }
  return { rawCount: raw.length, records: [...byId.values()], duplicates: raw.length - byId.size, pagesFetched, searchesRun };
}

module.exports = { ENDPOINT, TERMS, LIMITS, credentialError, safeAdzunaUrl, normalizeItem, requestPage, testConnection, fetchAdzunaJobs };
