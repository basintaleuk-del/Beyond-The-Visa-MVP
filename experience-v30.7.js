(()=>{
 ...
 if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=30.7').catch(error=>console.warn('Offline support:',error.message)),{once:true});
 ...
 function floatingZibur(){const button=$('#btvPlatformOpen');if(!button)return;if(button.dataset.ziburAction==='true')return;button.__memberOpen=button.onclick;button.dataset.memberOpenReady='true';originalMemberOpen=button.onclick;button.dataset.ziburAction='true';button.childNodes[0].textContent='Ask Zibur ✦ ';button.onclick=()=>window.openScreen?.('assistant')}
 function simplifyHome(){document.querySelectorAll('#home .quick button,#dashboardV3 button').forEach(button=>{const text=button.textContent.toLowerCase();if(text.includes('cost planner')){button.dataset.open='documents';button.innerHTML='<i>▧</i><span>My documents</span><small>Secure file vault</small>';button.dataset.uniqueShortcut='documents';return}if(text.includes('ask zibur')||text.includes('learning')||text.includes('premium membership')){button.hidden=true;button.dataset.duplicateNavigation='true'}});const duplicateDocument=[...document.querySelectorAll('#home .quick button')].filter(x=>x.textContent.toLowerCase().includes('my documents'));duplicateDocument.slice(1).forEach(x=>{x.hidden=true;x.dataset.duplicateNavigation='true'})}
