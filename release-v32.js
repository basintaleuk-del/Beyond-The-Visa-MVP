(()=>{
 ...
 const open=id=>{if(id==='articles'){q('#btvPlatformOpen')?.click();setTimeout(()=>q('[data-btv-pane="discover"]')?.click(),80);return}if(typeof window.openScreen==='function')window.openScreen(id);else location.href=`index.html#${id}`;setTimeout(()=>scrollTo({top:0,behavior:'smooth'}),30)};
 ...
 document.readyState==='loading'?document.addEventListener('DOMContentLoaded',wire):wire();setTimeout(wire,500);setTimeout(wire,1600);new MutationObserver(wire).observe(document.documentElement,{childList:true,subtree:true});
})();
