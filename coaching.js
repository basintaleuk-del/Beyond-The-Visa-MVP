...
function buildOnboarding(){
  ...
  section.innerHTML='<div class="onboardCard"><span class="onboardStep">PERSONALISE YOUR JOURNEY</span><h1>Tell us where you are heading</h1><p>Zibur will use these answers to tailor your nursing or midwifery checklist and next steps.</p><form id="onboardForm" class="onboardGrid"><label>Destination<select id="obDestination"><option value="uk">United Kingdom</option><option value="au">Australia</option><option value="ca">Canada</option><option value="nz">New Zealand</option><option value="ie">Ireland</option><option value="us">United States</option></select></label><label>Profession<select id="obProfession"><option>Nurse</option><option>Midwife</option></select></label><label>Country of qualification<input id="obQualification" required placeholder="e.g. Nigeria"></label><label>Registration stage<select id="obStage"><option>Just researching</option><option>Preparing documents</option><option>Application started</option><option>Tests in progress</option><option>Registration completed</option></select></label><label>Job-offer status<select id="obJob"><option>No job offer yet</option><option>Currently applying</option><option>Interview arranged</option><option>Job offer received</option></select></label><label>Travelling with dependants?<select id="obDependants"><option value="no">No</option><option value="yes">Yes</option><option value="unsure">Not sure yet</option></select></label><button class="onboardSubmit">Build my journey</button></form><small class="onboardPrivacy">You can change these answers later. This preview saves them on this device.</small></div>';
