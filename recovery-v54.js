(()=>{
 'use strict';
 if(window.__btvRecoveryV54)return;window.__btvRecoveryV54=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v54';
  ...
  if(!reset){
   ...
   const url=new URL(location.href);url.searchParams.set('v','54');url.searchParams.delete('refresh');url.searchParams.delete('home');url.searchParams.delete('screen');location.replace(url.href);return;
  }
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v54.js?v=54')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  ...
  [['Countries',window.renderCountries],['Journey',window.renderChecklist],['Costs',window.renderCosts]].forEach(([label,renderer])=>{if(typeof renderer==='function'){try{renderer()}catch(error){console.warn(label+' recovery:',error.message)}}});
  restoreDashboard();
 }
