...
test("the modern home has one Golden Question tile that opens the full experience", async () => {
  const [dashboard, golden] = await Promise.all([
    read("web/dashboard-premium-v73.js"),
    read("web/golden-question-v126.js"),
  ]);
  assert.equal((dashboard.match(/title: "Today’s Golden Question"/g) || []).length, 1);
  ...
  assert.match(golden, /Challenge, leaderboard, history and rules/);
