...
test("Jobs centre renders direct NHS vacancies for all staff families", async () => {
  ...
  assert.match(ui, /source_name",\["NHS Jobs","REED"\]/); assert.doesNotMatch(ui, /\bTrac\b|HealthJobsUK|NursingNetUK/i);
  assert.match(css, /nhsJobsLayout148/); assert.match(css, /nhsJobDetail150/); assert.match(shell, /job-application-v174\.js\?v=174[\s\S]*jobs-centre-v148\.js\?v=271[\s\S]*opportunity-centre-v138\.js\?v=253/);
