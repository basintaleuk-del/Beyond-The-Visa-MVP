...
test('loads the requested v71 merge after feature styles',()=>{assert.ok(index.indexOf('v71-feature-merge-v82.css')>index.indexOf('learning-analytics-v79.css'));assert.match(index,/release-v68\.js\?v=\d+(?:\.\d+)?/);assert.match(index,/release-v71\.js\?v=\d+(?:\.\d+)?/)});
test('loads the current premium dashboard rather than the superseded reference replacement',()=>{assert.match(index,/dashboard-premium-v73\.css/);assert.match(index,/dashboard-premium-v73\.js/);assert.doesNotMatch(index,/dashboard-reference-v74\.(?:css|js)/)});
