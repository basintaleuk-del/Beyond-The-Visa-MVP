...
test("public counts query published and closing-soon records only", async () => { const ui = await readFile(new URL("../web/opportunity-centre-v138.js", import.meta.url), "utf8"); assert.match(ui, /in\("status", ACTIVE_OPPORTUNITY_STATUSES\)\.is\("expired_at", null\)/); });
