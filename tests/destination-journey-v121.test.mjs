...
test('authenticated destination is loaded from the authoritative account column',()=>{
  assert.match(client,/select\('destination_country,destination'\)/);
...
test('destination UI changes only after RPC confirmation and rolls back on failure',()=>{
  const rpc=client.indexOf("rpc('btv_set_destination_country'");
  const apply=client.indexOf("apply('account-selection')",rpc);
  assert.ok(rpc>=0&&apply>rpc);
  assert.match(client,/Object\.assign\(model,previous\);apply\('save-rollback'\)/);
  assert.match(client,/setPickerBusy\(true\)/);
});

test('journey totals include only active required non-archived destination steps',()=>{
  assert.match(client,/is_active!==false&&step\.is_archived!==true&&step\.is_required!==false/);
