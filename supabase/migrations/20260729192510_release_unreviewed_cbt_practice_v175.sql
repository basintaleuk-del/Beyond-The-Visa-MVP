-- Release every non-rejected CBT question into secure practice circulation.
-- Clinical review status remains visible in the payload. Reviewed-only exam and
-- mock functions are intentionally unchanged.
...
begin
  ...
  return jsonb_build_object(
    ...
    'total', (
      ...
      where coalesce(q.quality_status, '') <> 'rejected'
    ),
    'reviewed', (
      select count(*) from public.cbt_questions q
      where coalesce(q.quality_status, '') <> 'rejected'
...
begin
  ...
  where q.id = p_question_id
    and coalesce(q.quality_status, '') <> 'rejected';
  if not found then
