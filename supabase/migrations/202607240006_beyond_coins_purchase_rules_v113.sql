-- Enforce live platform currency settings during every resource purchase.
...
begin
 ...
 if rule.maximum_daily_spend is not null then
  select coalesce(abs(sum(amount)),0) into spent_today from btv_wallet_transactions where user_id=uid and amount<0 and status='completed' and created_at>=(date_trunc('day',now() at time zone rule.reset_timezone) at time zone rule.reset_timezone);
  if spent_today+effective_price>rule.maximum_daily_spend then return jsonb_build_object('success',false,'code','DAILY_SPEND_LIMIT','message','This purchase exceeds your daily Beyond Coins spending limit. No coins were deducted.','limit',rule.maximum_daily_spend,'spent_today',spent_today); end if;
 end if;
 if w.balance<effective_price then return jsonb_build_object('success',false,'code','INSUFFICIENT_BALANCE','message',format('You need %s more Beyond Coins to unlock this resource.',effective_price-w.balance),'required',effective_price,'available',w.balance,'shortfall',effective_price-w.balance,'behavior',rule.insufficient_balance_behavior); end if;
 ...
 values(uid,prod.id,prod.access_type,prod.attempts,p_idempotency_key,case when expiry_days is null then null else now()+make_interval(days=>expiry_days) end) returning * into ent;
 ...
 return jsonb_build_object('success',true,'resumed',false,'entitlement_id',ent.id,'transaction_id',tx,'status','ready','balance',after_bal,'coin_price',effective_price,'product_code',prod.code,'route',prod.linked_route,'expires_at',ent.expires_at);
