(()=>{
 'use strict';
 if(window.__btvRecoveryV55)return;window.__btvRecoveryV55=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v55';
  ...
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v55.js?v=55')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  const shell=document.getElementById('appShell');if(!shell||shell.hidden||shell.dataset.recoveredV55==='yes')return false;
  shell.dataset.recoveredV55='yes';
