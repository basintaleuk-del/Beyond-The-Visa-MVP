(()=>{
 'use strict';
 if(window.__btvRecoveryV56)return;window.__btvRecoveryV56=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v56';
  ...
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v56.js?v=56')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  const shell=document.getElementById('appShell');if(!shell||shell.hidden||shell.dataset.recoveredV56==='yes')return false;
  shell.dataset.recoveredV56='yes';
