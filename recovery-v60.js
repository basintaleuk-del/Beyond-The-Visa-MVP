(()=>{
 'use strict';
 if(window.__btvRecoveryV60)return;window.__btvRecoveryV60=true;
 async function refreshBrowserCache(){
  const key='btv-browser-reset-v60';
  ...
  if(!reset){
   ...
   const url=new URL(location.href);url.searchParams.set('v','60');url.searchParams.delete('refresh');url.searchParams.delete('home');url.searchParams.delete('screen');location.replace(url.href);return;
  }
  try{if('serviceWorker'in navigator)await navigator.serviceWorker.register('./sw-v60.js?v=60')}catch(error){console.warn('Offline support:',error.message)}
 }
 refreshBrowserCache();
 ...
 function restoreSignedInContent(){
  const shell=document.getElementById('appShell');if(!shell||shell.hidden||shell.dataset.recoveredV60==='yes')return false;
  shell.dataset.recoveredV60='yes';
